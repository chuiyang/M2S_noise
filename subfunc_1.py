...
def draw_sigma(ytest, ypred, compound, name, SW_pic_output, run, bound=0.8):
    ...
    for i in range(len(compound)):
        ...
        if plot:
            plt.figure()
            # plt.hold(True)
            P_yture, = plt.plot(x, ytest[i, :], 'r')
            P_ypred, = plt.plot(x, ypred[i, :], 'b')
            plt.title(compound[i] +'  R2=' + '{:0.4f}'.format(r2_c[i]))
            plt.xlabel('Sigma')
            #plt.ylabel('')
            plt.legend([P_yture, P_ypred], ['Ture', 'Pred.'])
            createFolder('./Fig')
            createFolder('./Fig/' + name + '_run' +str(run))
            if r2_c[i]<bound:
                createFolder('./Fig/' + name + '_run' +str(run) + '/R2L' + str(bound))
                plt.savefig('./Fig/' + name+ '_run' +str(run) + "/R2L"+  str(bound) + "/" + compound[i] + ".png")
            else:
                createFolder('./Fig/' + name + '_run' +str(run) + '/R2G' + str(bound))
                plt.savefig('./Fig/' + name + '_run' +str(run) + "/R2G" + str(bound) + "/" + compound[i] + ".png")
            createFolder('./Fig/' + name + '_run' +str(run) + '/All')
            plt.savefig('./Fig/' + name + '_run' +str(run) + "/All/" + compound[i] + ".png")
            plt.close('all')
...
def draw_cluster_3D(y,compound,r2, name, bound=0.8, comp_name_show=1,r2_b=np.array([]),mark_a='',mark_b=''):
    ...
    if len(r2_b)!= 0:
        ax.scatter(y[r2_b < bound, 0], y[r2_b < bound, 1], y[r2_b < bound, 2], c='y', marker='o', label=mark_b + 'R2<' + str(bound))
    ax.scatter(y[r2<bound, 0], y[r2<bound, 1], y[r2<bound, 2], c='r', marker='o',label=mark_a + 'R2<' + str(bound))

    if comp_name_show:
        for i in range(max(int(len(compound)*0.02),20)):
            ax.text(y[shuffle_index[i], 0], y[shuffle_index[i], 1], y[shuffle_index[i], 2], compound[shuffle_index[i]] + '  R2=' + '{:0.4f}'.format(r2[shuffle_index[i]]), fontsize=5)
    ax.legend()
    #plt.show()
...
def draw_cluster_2D(y,compound,r2,name, bound=0.8, comp_name_show=1,r2_b=np.array([]),mark_a='',mark_b=''):
    ...
    if len(r2_b)!= 0:
        plt.scatter(y[r2_b < bound, 0], y[r2_b < bound, 1], c='chartreuse', marker='o', label=mark_b + 'R2<' + str(bound),s=5)
    ...
    if comp_name_show:
        for i in range(len(compound)):
            if r2[i]<bound:
                texts.append(plt.text(y[i, 0], y[i, 1], compound[i] + '  R2=' + '{:0.2f}'.format(r2[i]), fontsize=5, color='red'))
            if len(r2_b) != 0 and r2_b[i] < bound:
                texts.append(plt.text(y[i, 0], y[i, 1], compound[i] + '  R2=' + '{:0.2f}'.format(r2_b[i]), fontsize=5, color='green'))
...
def integral(y_true, y_pred):
    ...
    return K.mean(K.square(y_pred - y_true), axis=-1) + K.mean(K.square(Acc_L2R_t*dx-Acc_L2R_p*dx) + K.mean(K.square(Acc_R2L_t*dx-Acc_R2L_p*dx)), axis=-1)

def PCA_draw_2D(y,compound,r2,feature_1,feature_2,name, bound=0.8, comp_name_show=1,r2_b=np.array([]),mark_a='',mark_b=''):
    shuffle_index = np.arange(len(compound), dtype=np.int32)
    np.random.shuffle(shuffle_index)
    ...
    if len(r2_b)!= 0:
        plt.scatter(y[r2_b < bound, feature_1], y[r2_b < bound, feature_2], c='y', marker='o', label=mark_b + 'R2<' + str(bound),s=9)
    plt.scatter(y[r2 < bound, feature_1], y[r2 < bound, feature_2], c='r', marker='o',label=mark_a + 'R2<' + str(bound),s=9)
    texts = []

    if comp_name_show:
        for i in range(len(compound)):
            if r2[i]<bound:
                texts.append(plt.text(y[i, feature_1], y[i, feature_2], compound[i] + '  R2=' + '{:0.2f}'.format(r2[i]), fontsize=5, color='red'))
            if len(r2_b) != 0 and r2_b[i] < bound:
                texts.append(plt.text(y[i, feature_1], y[i, feature_2], compound[i] + '  R2=' + '{:0.2f}'.format(r2_b[i]), fontsize=5, color='green'))
...
def PCA_draw_3D(y,compound,r2, feature_1,feature_2,feature_3, name, bound=0.8, comp_name_show=1):
    ...
    ax.scatter(y[r2>=bound, feature_1], y[r2>=bound, feature_2], y[r2>=bound, feature_3], c='b', marker='o',label='R2>=' + str(bound))
    ax.scatter(y[r2<bound, feature_1], y[r2<bound, feature_2], y[r2<bound, feature_3], c='r', marker='o',label='R2<' + str(bound))
